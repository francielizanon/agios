/*! \file WFQ.c
    \brief Implements the WFQ scheduling algorithm
...
static bool g_wfq_first_req; /**< used to know when twins is being used for the first time (so we'll reset it) */
static int g_current_wfq_server; /**< the current queue from where we are taking requests */

/**
 * initialization of WFQ with ...
 * @return true if initialization successful
 */
bool WFQ_init(){
    g_wfq_first_req = true;
    g_current_wfq_server = 0; //starting with first queue
    return true;
}
