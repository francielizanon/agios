...
void *test_thr(void *arg)
{
	...
	if (me == 0) my_function = test_process_th1;
	else if (me == 1) my_function = test_process_th2;
	else if (me == 2) my_function = test_process_th3;
	else {
		assert (me == 3);
		my_function = test_process_th4;
	}
...
void retrieve_arguments_and_generate_requests(int argc, char **argv)
{
	...
	if (argc < 5) {
		printf("Usage: ./%s <number of requests per thread> <requests' size in bytes> <time between requests in ns> <time to process requests in ns>\n", argv[0]);
		exit(1);
	}
	g_thread_nb=4;
...
int main (int argc, char **argv)
{
	...
	// allocate the vector of request-processing threads
